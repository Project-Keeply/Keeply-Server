...
> 이 스킬은 표준 Flight Protocol의 **Flight** 단계입니다. 실행 전 [Preflight](../../checklists/preflight.md), 실행 후 [Postflight](../../checklists/postflight.md) → [Debrief](../../checklists/debrief.md)를 따르세요. (본 스킬은 코드를 직접 수정하지 않으므로 Postflight의 Gradle 검증은 N/A로 처리)
...
본 프로젝트는 `develop`을 기본 base 브랜치로 사용한다 (`docs/rules/git-convention.md` Git Flow 참고).
...
- `develop` 브랜치에서 작업 중이면(예: 릴리스 준비) `main`을 base로 대체
